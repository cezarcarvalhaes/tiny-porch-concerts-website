...
function PorchInfo({ porchInfo = [] }: { porchInfo: EventInfo['porches'] }) {
	return (
		<Flex flexDirection="column" w="full">
			...
			{porchInfo.map(({ address = '', performers = [] }) => (
				<Stack key={address} mb={4} backgroundColor="brand.yellow" p={6} maxW="30em" borderRadius={12}>
					<Heading variant="secondary">
						{address}
						<Icon as={FaHome} ml={2} />
					</Heading>
					{performers.map(({
						name, link, time, venmo_link,
					}) => (
						<PorchBox
							key={`${name}-${time}`}
							performer={name}
